...
public class HintSystem extends ASystem {
    ...
    public void handleEvent(Event event) {
        switch (event.getTopic()) {
            case MAKE_HINT: onMakeHint(event); break;
        }
    }

    private void onMakeHint(Event event) {
        ...
        if(hints.containsKey(key)) {
            Entity hint = hints.get(key);
            Detectable d = hint.getComponent(Detectable.class);
            d.setStrength(d.getStrength() + 1);
            hint.getComponent(Visual.class).setAlpha((float)d.getStrength() * 0.1f);
